...
func (receiver *MyStr) Test1(name string) {
	receiver.myName = name
	fmt.Println("Hello " + receiver.myName)
}
...
func dynamicInvoke() {
	...
	myStr := &MyStr{}
	in := []reflect.Value{reflect.ValueOf("Tom")}
	myStrValue := reflect.ValueOf(myStr)
	myStrValue.MethodByName("Test1").Call(in)
	fmt.Println(reflect.ValueOf(myStr).Elem().FieldByName("myName").String())
}
...
func init() {
	...
	fileTypeMap.Store("255044462d312e", "pdf")        //Adobe Acrobat (pdf)255044462d312e360d25
...
func GetFileType() {
	f, err := os.Open("C:\\Users\\Administrator\\Desktop\\数据结构语言版第三版_邓俊辉.pdf")
	if err != nil {
		panic(err)
	}
